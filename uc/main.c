...
void generuj_raport_stanu_urzadzenia(void) {
  ...
  *sms++ = '\n';
  ...
  // Status sterownika (START/STOP)
  if (blokada_systemu) {
    strcpy_P((char *)sms, PSTR("Status: Zablokowany"));
  } else {
    strcpy_P((char *)sms, PSTR("Status: Aktywny"));
  }
  sms += strlen((char *)sms);
  *sms++ = '\n';

  // Tryb pracy (OPEN/CLOSE)
  if (tryb_pracy == 0) {
...
void wykonanie_polecenia_sms(void) {
  ...
  if (blokada_systemu) {
    ...
    if (komenda != INTERPRETACJA_SMS_POPRAWNY && // START/STOP zwracaja POPRAWNY
      ...
      if (eeprom_read_byte((const uint8_t *)ADRES_EEPROM_BLOKADA_SYSTEMU) ==
          1) {
        zapisz_debug_do_eeprom(1, 2); // Debug: Odrzucono przez blokade
  ...
  case INTERPRETACJA_SMS_DEBUG: {
    // DEBUG - wyślij flagi debug dla SKRYBA i USER
    strcpy_P((char *)tekst_wysylanego_smsa, PSTR("DEBUG\n"));
    uchar *sms = tekst_wysylanego_smsa + strlen((char *)tekst_wysylanego_smsa);

    // SKRYBA DEBUG
    strcpy_P((char *)sms, PSTR("SKRYBA:"));
    sms += strlen((char *)sms);
    *sms++ =
        '0' + eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_SKRYBA_2);
    *sms++ =
        '0' + eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_SKRYBA_3);
    *sms++ =
        '0' + eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_SKRYBA_4);
    *sms++ =
        '0' + eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_SKRYBA_5);
    *sms++ = '\n';

    // USER DEBUG - wyświetl jako liczby dziesiętne
    strcpy_P((char *)sms, PSTR("U1="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_1), sms, 10);
    sms += strlen((char *)sms);

    strcpy_P((char *)sms, PSTR(" U2="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_2), sms, 10);
    sms += strlen((char *)sms);

    strcpy_P((char *)sms, PSTR(" U3="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_3), sms, 10);
    sms += strlen((char *)sms);
    *sms++ = '\n';

    strcpy_P((char *)sms, PSTR("U4="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_4), sms, 10);
    sms += strlen((char *)sms);

    strcpy_P((char *)sms, PSTR(" U5="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_5), sms, 10);
    sms += strlen((char *)sms);

    strcpy_P((char *)sms, PSTR(" U6="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_6), sms, 10);
    sms += strlen((char *)sms);

    strcpy_P((char *)sms, PSTR(" U7="));
    sms += strlen((char *)sms);
    utoa(eeprom_read_byte((const uint8_t *)ADRES_EEPROM_DEBUG_USER_7), sms, 10);
    sms += strlen((char *)sms);
    *sms++ = '\n';

    strcpy_P((char *)sms, PSTR("1=RCV 2=BSY 3=CNT 4=QSM 5=QAL 6=FND 7=ADD"));
  ...
  case INTERPRETACJA_SMS_USER: {
    ...
    zapisz_debug_do_eeprom(2, znaleziono); // 2 = USER
    break;
  }
...
uchar wykonanie_komend_ukladow(void) {
  ...
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_32:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_33:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_34:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_35:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_36:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_37:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_38:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_39:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_40:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_41:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_42:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_43:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_44:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_45:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_46:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_47:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_48:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_49:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_50:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_51:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_52:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_53:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_54:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_55:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_56:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_57:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_58:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_59:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_60:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_61:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_62:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_63:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_64:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_65:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_66:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_67:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_68:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_69:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_70:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_71:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_72:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_73:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_74:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_75:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_76:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_77:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_78:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_79:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_80:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_81:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_82:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_83:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_84:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_85:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_86:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_87:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_88:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_89:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_90:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_91:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_92:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_93:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_94:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_95:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_96:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_97:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_98:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_99:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_100:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_101:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_102:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_103:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_104:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_105:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_106:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_107:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_108:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_109:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_110:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_111:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_112:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_113:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_114:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_115:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_116:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_117:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_118:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_119:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_120:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_121:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_122:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_123:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_124:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_125:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_126:
  case KOMENDA_KOLEJKI_RESET_USTAWIEN_127: {
    JESLI_EEPROM_ZAJETY_WYKONAJ_POZNIEJ();
...
#define LICZBA_BAJTOW_ZAPISYWANA_W_JEDNEJ_KOMENDZIE 32
  ...
  case KOMENDA_KOLEJKI_SPRAWDZ_UZYTKOWNIKOW_BRAMA: {
    ...
    zapisz_znak_w_eeprom(skryba_wlaczona ? 1 : 0,
                         ADRES_EEPROM_DEBUG_SKRYBA_2); // DEBUG
    zapisz_znak_w_eeprom(znaleziono ? 0 : 1,
                         ADRES_EEPROM_DEBUG_SKRYBA_3); // DEBUG

    if (skryba_wlaczona && !znaleziono) {
      uchar len_full = strlen((char *)numer_lokalny);

      // DEBUG: Zapisz dlugosc numeru
      zapisz_znak_w_eeprom(len_full, ADRES_EEPROM_DEBUG_SKRYBA_5);
      ...
      if (len_full >= 3) {
        ...
        if (strlen((char *)numer_telefonu_skryba) >= 3) {
          zapisz_znak_w_eeprom(
              1, ADRES_EEPROM_DEBUG_SKRYBA_4); // DEBUG: komenda dodana
...
void inicjalizuj_parametry_modulu(void) {
  ...
  zapisz_debug_do_eeprom(skryba_wlaczona ? 1 : 0, (uchar)(skryba_limit & 0xFF));
  ...
  // Inicjalizacja zmiennych debug SKRYBA (jesli 0xFF to wyczysc)
  for (uint16_t addr = ADRES_EEPROM_DEBUG_SKRYBA_1;
       addr <= ADRES_EEPROM_DEBUG_SKRYBA_5; ++addr) {
    if (eeprom_read_byte((const uint8_t *)addr) == 0xFF) {
      while (!eeprom_is_ready())
        ;
  ...
  // Inicjalizacja zmiennych debug USER (jesli 0xFF to wyczysc)
  for (uint16_t addr = ADRES_EEPROM_DEBUG_USER_1;
       addr <= ADRES_EEPROM_DEBUG_USER_7; ++addr) {
    if (eeprom_read_byte((const uint8_t *)addr) == 0xFF) {
      while (!eeprom_is_ready())
        ;
      eeprom_update_byte((uint8_t *)addr, 0);
