          ...
          if ((p3 = strchr(p3, ':')) != NULL && p2 > p3++) {
            ...
            if (ptr_time) {
              ...
              if (strlen(ptr_time) >= 8) {
                ...
                // Aktualizacja blokady czasowej
                if (czas_start_h == 0xFF) {
                  blokada_sterowania_czasowa = FALSE;
                } else {
                  // Parsowanie aktualnego czasu
                  int curr_h = (rtc_czas[0] - '0') * 10 + (rtc_czas[1] - '0');
                  int curr_m = (rtc_czas[3] - '0') * 10 + (rtc_czas[4] - '0');
                  ...
                  if (start_time <= stop_time) {
                    // Normalny przedzial (np. 08:00 - 16:00)
                    if (curr_time >= start_time && curr_time <= stop_time)
                      blokada_sterowania_czasowa = FALSE;
                    else
                      blokada_sterowania_czasowa = TRUE;
                  } else {
                    // Przejscie przez polnoc (np. 22:00 - 06:00)
                    if (curr_time >= start_time || curr_time <= stop_time)
                      blokada_sterowania_czasowa = FALSE;
                    else
                      blokada_sterowania_czasowa = TRUE;
...
void obsluga_komendy_SIM900(void) {
  ...
  if (czy_jest_komenda_rozmowa_telefoniczna()) {
    POMOC_DODAJ2('#', 'b');
    // numer_telefonu_ktory_dzwoni[0] = '\0';  // REMOVED: Niepotrzebne
    // czyszczenie - numer jest zaraz nadpisywany
  ...
  if (ptr_dtmf != NULL) {
    ...
    if (!tryb_clip) // Tylko w trybie DTMF
    {
      if (dtmf_key == '1') {
        stan_wyjscie[0] = TRUE;
        licznik_przelacznik_wyjscia[0] = 2 * 10ul; // 2 sekundy retrigger
        zapal_diode_led(10);
      }
    }
  ...
  if (czy_jest_komenda_otrzymano_sms_flash()) {
    ...
    if (p1++ != NULL) {
      ...
      if (p2 != NULL) {
        ...
        // strncpy(&tekst_wysylanego_smsa[1], p1, MAX_LICZBA_ZNAKOW_SMS - 1);
        // tekst_wysylanego_smsa[MAX_LICZBA_ZNAKOW_SMS] = 0;
      }
    }
    return;
  }
...
uchar wykonanie_komend_SIM900(void) {
  ...
  case KOMENDA_KOLEJKI_ODRZUC_ROZMOWE: {
    POWTORZ_JESLI(
        flaga_wysylanie_smsa || wykonywanie_rozmowy_telefonicznej ||
        aktualnie_wysylane_polecenie_SIM900 != KOMENDA_KOLEJKI_BRAK_KOMENDY ||
        czekanie_na_odebranie_zachety || wykonywanie_rozmowy_telefonicznej ||
        !CZY_MOZNA_WYSYLAC_DANE_SIM900() || !czy_jest_bezczynny_SIM900());
    wysylane_polecenie_SIM900 = wykonywana_komenda;
