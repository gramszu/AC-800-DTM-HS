...
#define ascii_to_pdu(bufor_pdu, ascii)	ascii_to_pdu_blok(bufor_pdu, ascii, ascii + strlen((const char*) ascii))
uchar* ascii_to_pdu_blok(uchar* bufor_pdu, const uchar* ascii_beg, const uchar* ascii_end);
void konwertuj_blok_dwa_znaki_na_znak_pdu(const uchar* blok_danych, const uchar liczba_znakow, uchar* ptr);
...
void konwertuj_pdu_na_blok_wysylany(uchar* buf, const uchar* pdu_ptr, const uchar dlugosc_pdu);
